import { DiffHunk, DiffLine, FileChangeWithHunks } from "@/shared/types/git";

export type StagedLineSelection = {
  [hunkIdx: number]: Set<number>;
  isNewFile?: boolean;
  isWholeFileStaged?: boolean;
};

export type StagedLinesByFile = Record<string, StagedLineSelection>;
...
export function buildSyncedStagedLinesState(
  ...
  const nextState: StagedLinesByFile = {};
