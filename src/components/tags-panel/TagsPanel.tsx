...
import type { TagCommitOption } from "../../types/git";
...
export function TagsPanel({ repoPath }: TagsPanelProps) {
  ...
  useEffect(() => {
    ...
    core
      .invoke<TagCommitOption[]>("search_tag_commits", {
        path: repoPath,
        query: debouncedCommitSearch,
        limit: 50,
      })
      .then((commits) => {
  ...
  const handleCreateTag = useCallback(async () => {
    ...
    try {
      await core.invoke("create_tag", {
        path: repoPath,
        tagName: tagName.trim(),
        commitSha: selectedCommit.sha,
        annotated: tagAnnotated,
        description: tagAnnotated ? tagDescription.trim() : null,
      });
