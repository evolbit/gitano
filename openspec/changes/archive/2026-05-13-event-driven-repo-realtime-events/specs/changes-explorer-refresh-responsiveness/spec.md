...
#### Scenario: Realtime refresh returns the same file snapshot
- **WHEN** a working-changes refresh runs while the current changes explorer is visible in response to repository change events
...
- **THEN** the explorer MUST NOT reset the scroll position or selection state solely because a refresh trigger fired
...
- **AND** a repository-change-triggered refresh completes with no meaningful snapshot change
- **THEN** the explorer MUST continue scrolling smoothly
- **THEN** the explorer MUST NOT visibly jump, stutter, or re-anchor the list solely due to that unchanged refresh

### Requirement: Working-changes refresh updates the explorer when the file snapshot meaningfully changes
The system SHALL update the current changes explorer when a repository-change-triggered refresh detects meaningful changes in the working tree.
...
- **WHEN** a repository-change-triggered refresh completes and the staged selection state for the visible files is unchanged
