...
#### Scenario: Commit message column expands into available space
- **WHEN** the commit list table is rendered in the workspace
- **THEN** the message column MUST use the remaining available horizontal space after fixed-width columns are laid out
- **THEN** supporting columns such as SHA, date, author, history, and file count MUST retain explicit sizing constraints

#### Scenario: Column resize handles are shown
- **WHEN** the commit list table allows column resizing
- **THEN** the resize handles MUST remain usable
- **THEN** the visible handle treatment MUST be lighter than the current default so the separators feel thinner
