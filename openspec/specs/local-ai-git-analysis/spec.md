...
The system SHALL resolve the model for each local AI action from the action-specific preference, global preference, or registry default before execution.
...
- **WHEN** a local AI action does not have an action-specific model preference
- **THEN** the backend MUST use the persisted global default model

#### Scenario: No model preference exists
- **WHEN** no action-specific or global model preference exists
- **THEN** the backend MUST use the registry recommended default model
